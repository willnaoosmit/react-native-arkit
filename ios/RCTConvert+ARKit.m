...
+ (SCNLine * )SCNLine:(id)json {
    ...
    if(pointCount != 2 ){
      NSLog(@"Currently only supporting lines with 2 points");
    ...
    SCNVector3 endPoint = positions[1];
    SCNVector3 startPoint = positions[0];
    ...
    [self addMaterials:cyl json:json sides:3];
    mainParentNode = [SCNNode nodeWithGeometry:cyl];


    //original vector of cylinder above 0,0,0
    SCNVector3 ov = SCNVector3Make(0.0, length/2.0, 0.0);
    //target vector, in new coordination
    SCNVector3 nv = SCNVector3Make((endPoint.x - startPoint.x)/2.0,
    ...
    GLKVector3 av_normalized = GLKVector3Normalize(GLKVector3Make(av.x, av.y, av.z));
    float q0 = (float)(0.0); //cos(angel/2), angle is always 180 or M_PI
    float q1 = (float)(av_normalized.x); // x' * sin(angle/2)
    float q2 = (float)(av_normalized.y); // y' * sin(angle/2)
    float q3 = (float)(av_normalized.z); // z' * sin(angle/2)

    // Build the transform matrix to set on the mainParentNode
    SCNMatrix4 mat = SCNMatrix4Identity;
    mat.m11 = q0 * q0 + q1 * q1 - q2 * q2 - q3 * q3;
    mat.m12 = 2 * q1 * q2 + 2 * q0 * q3;
    mat.m13 = 2 * q1 * q3 - 2 * q0 * q2;
    mat.m21 = 2 * q1 * q2 - 2 * q0 * q3;
    mat.m22 = q0 * q0 - q1 * q1 + q2 * q2 - q3 * q3;
    mat.m23 = 2 * q2 * q3 + 2 * q0 * q1;
    mat.m31 = 2 * q1 * q3 + 2 * q0 * q2;
    mat.m32 = 2 * q2 * q3 - 2 * q0 * q1;
    mat.m33 = q0 * q0 - q1 * q1 - q2 * q2 + q3 * q3;
    mat.m41 = (startPoint.x + endPoint.x) / 2.0;
    mat.m42 = (startPoint.y + endPoint.y) / 2.0;
    mat.m43 = (startPoint.z + endPoint.z) / 2.0;
    [mainParentNode setTransform:mat];
